...
def main() -> None:
    ...
    ops.append(
        para(
            "群聊触发的身份和行动提示会优先通过 QQ 群临时会话发送；群设置允许群友私聊且 NapCat 支持时，未加好友也能收到。为保证长期稳定收取通知，仍建议把机器人加为好友。"
        )
    )
    ops.append(
        para(
    ...
    ops.append(para("夜里没私聊，先确认群设置允许群友私聊，并检查 NapCat 是否成功发送群临时会话。若协议拦截，再把机器人加为好友。已经是好友却没有菜单，多半是你这身份夜里没技能，等天亮。"))
