...
class PostgreSQLStore:
    ...
    async def pending_deliveries(self, limit: int = 50, max_attempts: int = 5) -> list[DeliveryRecord]:
        now = datetime.now(timezone.utc)
        stale = now - timedelta(minutes=10)
        async with self.pool.acquire() as connection:
            async with connection.transaction():
                # 卡在 sending 超过 10 分钟基本是进程被杀，回收成 retry 让它继续走。
                # 但必须带 attempts 保护：已经用满重试次数的记录再放回队列，
                # 就变成「同一条消息被无限重投」，既刷屏又把 failed 越堆越多。
                await connection.execute(
                ...
                await connection.execute(
                    "UPDATE deliveries SET status='failed', next_attempt_at=NULL, updated_at=$1, "
                    "last_error=COALESCE(last_error, '发送中途进程退出，且已用满重试次数') "
                    "WHERE status='sending' AND updated_at < $2 AND attempts >= $3",
                    now,
                    stale,
                    int(max_attempts),
    ...
    async def purge_finished_deliveries(self, max_age_seconds: int = 86400) -> int:
        ...
        now = datetime.now(timezone.utc)
        cutoff = now - timedelta(seconds=max(600, int(max_age_seconds)))
        async with self.pool.acquire() as connection:
            rows = await connection.fetch(
                "DELETE FROM deliveries WHERE status IN ('sent','dead') AND updated_at < $1 "
    ...
    async def touch_player(self, user_id: str, name: str | None = None) -> None:
        ...
        名字为空串时按「没拿到」处理：QQ 群消息经常不下发昵称，写空串会把
        之前拿到过的真实昵称覆盖掉，玩家名单立刻退化成一片空白。
        """
    ...
    async def mark_delivery_dead(self, delivery_id: str, error: str) -> None:
        """明确不可能成功的投递（如「主动消息失败, 无权限」）一次判死。

        这类错误重试多少次都是同样的结果，继续退避只会把 failed 越堆越高，
        还会掩盖真正需要人工关注的临时故障。
        """
        now = datetime.now(timezone.utc)
        ...
        async with self.pool.acquire() as connection:
            await connection.execute(
                "UPDATE deliveries SET status='dead', attempts=attempts+1, last_error=$1, "
                "next_attempt_at=NULL, updated_at=$2 WHERE delivery_id=$3",
                safe_error,
                now,
                delivery_id,
            )
