...
def is_opaque_display_name(name: str) -> bool:
    ...
    NapCat 下昵称通常都有，但退群、风控等情况仍可能取到空串；另外历史存档里
    还留着官方开放平台时代的 32 位 openid。这两种都必须视为不可展示，
    否则群里就会出现 `607E22D6C8ACB3232D40F1DBF4D11947` 这种内部标识，
    或者一片空白。
    """
    text = (name or "").strip()
    if not text:
...
def is_real_qq_number(value: str) -> bool:
    ...
    NapCat 下 user_id 一定是真实 QQ 号，这里只用于校验管理指令里手输的号码。
    """
    return bool(re.fullmatch(r"[1-9][0-9]{4,14}", (value or "").strip()))
...
class Player:
    ...
    def short_label(self) -> str:
        ...
        只有座位号和 QQ 昵称两段，绝不出现 QQ 号、openid 或任何内部标识；
        昵称缺失时退化成纯座位号「1号」。
        """
        if is_opaque_display_name(self.display_name):
            return f"{self.seat}号"
